...
## 색상 시스템
- Primary (메인): [사용자가 언급한 브랜드 컬러를 hex 코드로 변환]
...
## 레이아웃 시스템
- 그리드: [사용자가 언급한 레이아웃 방식 (카드/리스트/그리드) 반영]
- 여백 단위: 8px 기준 (8px, 16px, 24px, 32px, 40px)
- 컨테이너 최대 너비: [적절한 크기]px
- 화면 좌우 패딩: 모바일 16px, 데스크톱 24px

## 핵심 화면별 구성
[사용자가 언급한 각 핵심 화면에 대해:]
### [화면 이름]
- 상단: [구성요소]
- 중앙: [구성요소]
- 하단: [구성요소]
...
## 사용자 여정별 화면 흐름
[사용자가 설명한 여정을 각 단계별로 구체화:]
1. [단계명]: [화면 구성 + 주요 UI 요소 + 사용자 액션]
2. [단계명]: [화면 구성 + 주요 UI 요소 + 사용자 액션]
...

**중요**:
1. 모든 색상은 hex 코드로 명시
2. 모든 크기는 px 단위로 명시
3. 사용자가 언급한 참고 앱의 디자인 패턴을 최대한 반영
4. 개발자가 바로 코드를 작성할 수 있을 정도로 구체적으로 작성
5. 사용자가 언급한 전체 분위기(미니멀/친근함/신뢰감 등)를 모든 요소에 일관되게 반영
6. 핵심 화면과 사용자 여정을 통합하여 각 화면에서의 실제 사용자 경험을 구체화`;
