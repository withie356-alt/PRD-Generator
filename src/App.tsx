...
export default function PRDPromptGenerator() {
  ...
  const generateDetailedQuestions = (): Question[] => {
    return [
      ...
      {
        question: "[기술 스택 & 구현 방식] 이 앱을 어떤 기술로 만들고 싶으신가요?",
        hint: "💡 잘 모르겠다면 입력 안 해도 됩니다! 기본값(Vite + React, localStorage)으로 자동 설정됩니다.\n\n🛠 개발 방식 (택 1):\n\n• Vite + React ⭐ 추천\n  → 설명: 화면을 빠르게 만들 수 있는 최신 도구\n  → 장점: 개발 속도 빠름, 무료 배포 쉬움 (Vercel/Netlify)\n  → 단점: 검색엔진 노출 약함\n  → 추천 대상: 처음 만들어보는 분, 빠르게 프로토타입 보고 싶은 분\n\n• Next.js\n  → 설명: React + 검색엔진 최적화 기능 포함\n  → 장점: 구글 검색 노출 좋음, 대규모 서비스 가능\n  → 단점: 초기 설정 복잡, 학습 곡선 있음\n  → 추천 대상: 블로그/쇼핑몰처럼 검색 노출 중요한 서비스\n\n• HTML/CSS/JS\n  → 설명: 가장 기본적인 웹 기술\n  → 장점: 배우기 쉬움, GitHub Pages 무료 배포\n  → 단점: 복잡한 기능 구현 어려움\n  → 추천 대상: 간단한 랜딩 페이지, 포트폴리오\n\n📦 데이터 저장 (택 1):\n\n• localStorage ⭐ 추천\n  → 설명: 사용자의 브라우저에만 데이터 저장\n  → 장점: 무료, 서버 불필요, 구현 간단\n  → 단점: 브라우저 삭제 시 데이터 사라짐, 다른 기기에서 접근 불가\n  → 추천 대상: 개인용 도구, 프로토타입\n\n• Firebase/Supabase\n  → 설명: 구글/오픈소스에서 제공하는 클라우드 데이터베이스\n  → 장점: 여러 사람이 데이터 공유 가능, 실시간 동기화\n  → 단점: 무료 한도 초과 시 유료, 외부 서비스 의존\n  → 추천 대상: 팀 협업 도구, 여러 기기에서 접속하는 서비스\n\n• PostgreSQL/MongoDB\n  → 설명: 직접 서버를 구축하고 데이터베이스 운영\n  → 장점: 완전한 제어, 데이터 소유권\n  → 단점: 서버 비용, 운영 지식 필요, 보안 관리 필요\n  → 추천 대상: 대규모 서비스, 민감한 데이터 다루는 경우\n\n📝 기본 설정 (자동 적용):\n• PRD 문서는 /docs/PRD.md 파일로 저장됩니다\n• 작업 내역은 /docs/TODOs.md에 기록하며 진행합니다\n• 테스트 코드, 실제 DB 연동, 외부 API는 일단 TODO에만 기록하고 나중에 구현합니다\n• 초기 버전은 가짜 데이터(mock)로 화면만 먼저 만들어 봅니다\n\n💡 입력 예시:\n\"Vite + React로 빠르게 만들어보고 싶어요. 데이터는 일단 localStorage에 저장하고, 나중에 사용자가 많아지면 Firebase로 바꿀게요.\""
      }
    ];
  };
...
3번째: [기술 스택 & 구현 방식] 기술 선택과 데이터 저장 방식
   "이 앱을 어떤 기술로 만들고 싶으신가요?

   개발 방식 예시:
   - Vite + React (빠른 프로토타입, 간단한 배포) ⭐ 추천
   - Next.js (검색 노출 필요, 대규모 서비스)
   - HTML/CSS/JS (정적 페이지, GitHub Pages)

   데이터 저장 예시:
   - localStorage (간단, 무료, 브라우저 로컬 저장) ⭐ 추천
   - Firebase/Supabase (클라우드 DB, 데이터 공유)
   - PostgreSQL/MongoDB (서버 직접 구축)

   기본 설정 (자동 적용):
   - PRD는 /docs/PRD.md에 저장
   - 작업 내역은 /docs/TODOs.md에 기록
   - 테스트 코드, DB, 외부 API는 TODO에만 기록
   - 데이터는 mock으로, API는 console.log로 대체"
