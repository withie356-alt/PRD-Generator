...
위 형식을 정확히 따라 ${userAnswers + 1}번째 질문을 생성하세요.`;
  ...
  const enrichDesignWithAI = async () => {
    ...
    const coreScreens = userAnswers[1]?.content || ''; // 핵심 화면
    const userJourney = userAnswers[2]?.content || ''; // 사용자 여정
...
- 네비게이션: 상단 검색 + 필터`
    ...
    for (const [appName, guide] of Object.entries(referenceAppGuides)) {
      if (designSystem.includes(appName) || coreScreens.includes(appName) || userJourney.includes(appName)) {
        appGuide = guide;
        break;
      }
    }
...
1. [단계명]: [화면 구성 + 주요 UI 요소]
2. [단계명]: [화면 구성 + 주요 UI 요소]
...
